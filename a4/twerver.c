...
void remove_client(struct client **clients, int fd)
{
    ...
    if (curr != NULL)
    {
        struct client *temp;
        if (prev != NULL)
        { // curr is not the start of the list
            temp = prev->next;
            prev->next = temp->next;
        }
        else
        { // Prev is null so curr is the start of the list
            *clients = curr->next;
            temp = curr;
        }
...
int verify_input(char *input)
{
    ...
    if (space == NULL)
    { // Command is either show, quit or invalid command
